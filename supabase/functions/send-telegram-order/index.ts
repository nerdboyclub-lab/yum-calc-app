...
serve(async (req) => {
  ...
  try {
    ...
    // Build message
    const paymentLabels: Record<string, string> = { card: 'Карта', cash: 'Наличные', payme_click: 'Payme/Click' };
    const paymentLabel = paymentLabels[latestPaymentMethod] || latestPaymentMethod;
